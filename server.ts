...
import { CyberKnightDB } from "./server/dbSim";
    ...
    if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
      ...
      try {
        ...
        try {
          ...
          throw new Error(`Failed to parse extracted JSON block. Error: ${innerErr.message || innerErr}`);
        }
      }
    }
    throw err;
...
app.post("/api/auth/signup", async (req, res) => {
  ...
  if (!/^[a-zA-Z0-9]{4,15}$/.test(rollNumber)) {
    return res.status(400).json({ success: false, error: "User ID must be alphanumeric and 4-15 characters long." });
  }
  ...
  if (!passwordRegex.test(password)) {
    return res.status(400).json({
      success: false,
      error: "Password must be 6-12 characters, including at least one uppercase letter, one lowercase letter, and one number."
    });
  }
...
Do not wrap the output in markdown block code tags other than json. Return strictly the raw JSON object.`;
    ...
    res.json({ success: true, message: "Signup successful. Welcome, Cyber Knight!" });
  } else {
...
app.post("/api/auth/login", (req, res) => {
  ...
  if (!rollNumber || !password) {
    return res.status(400).json({ success: false, error: "Please enter both Roll Number and Password." });
  }
...
app.post("/api/auth/forgot", (req, res) => {
  ...
  if (result.success) {
    res.json({ success: true, message: "Password reset successful! Reconnect email notification sent." });
  } else {
...
app.post("/api/events/register", (req, res) => {
  ...
  if (!rollNumber || !eventId) {
    return res.status(400).json({ success: false, error: "Missing roll number or event ID" });
  }
...
app.post("/api/clubs/join", (req, res) => {
  ...
  if (!clubName) {
    return res.status(400).json({ success: false, error: "Missing club name" });
  }
...
app.post("/api/clubs/leave", (req, res) => {
  ...
  if (result.success) {
    res.json({ success: true, message: "Resigned club membership successfully!" });
  } else {
    res.status(400).json({ success: false, error: result.error });
  }
});
...
app.get("/api/registrations/:rollNumber", (req, res) => {
  ...
  const sqlResult = db.executeSQL("SELECT * FROM registrations WHERE roll_number = ?", [rollNumber]);
  if (sqlResult.success) {
...
app.post("/api/sql/execute", (req, res) => {
  ...
  if (!sql) {
    return res.status(400).json({ success: false, error: "No SQL statement provided." });
  }
...
app.post("/api/helpdesk", async (req, res) => {
  ...
  if (!message || !rollNumber) {
    return res.status(400).json({ success: false, error: "Missing message or roll number." });
  }
...
For help with course registration, fees, or exam schedules, please visit the Academic Dean's office or submit an official inquiry once connection is restored!`;
  ...
  try {
    ...
    res.json({ success: true, reply: response.text || "I'm here to help, but no text was returned. Please try asking again!" });
  } catch (err: any) {
    console.error("Helpdesk Gemini API error:", err);
    res.status(500).json({ success: false, error: err.message, reply: `I'm having trouble connecting to the assistance neural link, but here are your local credentials: you are registered under the ${profile.department} and your faculty advisor is ${profile.facultyIncharge.name}.` });
  }
});

// AI Chat Assistant utilizing Gemini Live or Generate Content
...
`;
  ...
  try {
    ...
    const reply = response.text || "Connection secure, but no signal returned from command node. Try again!";
    res.json({ success: true, reply });
  } catch (error: any) {
    console.error("Gemini API calling error:", error);
    res.status(500).json({ success: false, error: "Command node overload. Falling back to local offline response.", reply: "I'm having trouble reaching the neural network grid. However, you can register for events, check faculty, or run queries using the local tabs above!" });
  }
});
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[Cyber Knight Server] Operational at http://0.0.0.0:${PORT}`);
  });
}
