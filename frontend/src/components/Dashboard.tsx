          ...
          >
            ...
            {activeTab === 'map' && (
              <div className="space-y-6">
                <div className="bg-cyber-slate/40 border border-cyber-blue/10 rounded-xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    <h3 className="text-sm font-display font-bold text-white uppercase tracking-wider">Campus Map Schematic</h3>
                    <p className="text-[11px] text-gray-400 font-mono mt-0.5">Tactical layout map displaying departments, administrative blocks, and club workspaces.</p>
                  </div>
                  <div className="text-xs font-mono px-3 py-1 bg-cyber-blue/10 border border-cyber-blue/25 text-cyber-blue rounded-lg font-bold">
                    📍 CURRENT INGRESS ZONE: {getStudentProfile(user.rollNumber).deptCode} WING
                  </div>
                </div>

                {/* Blueprint Navigation Router Panel */}
                <div className="bg-cyber-slate/30 border border-cyber-blue/15 rounded-xl p-4.5 space-y-3 font-mono">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-2.5">
                    <span className="text-[10px] text-cyber-blue font-bold uppercase tracking-wider flex items-center gap-1.5">
                      🗺️ GPS Blueprint Navigation Router
                    </span>
                    <span className="text-[9.5px] text-gray-400">
                      Select target destination node to view safe campus routing path.
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    <div className="md:col-span-2">
                      <select
                        ...
                        className="w-full bg-cyber-dark border border-cyber-blue/20 hover:border-cyber-blue/45 text-xs text-white rounded-lg p-2.5 cursor-pointer focus:outline-none"
                      >
                        <option value="">-- CHOOSE DESTINATION CORNER --</option>
                        <option value="cse">Computer Science (CSE) Block - Rooms 301-305</option>
                        <option value="ece">Electronics & Communication (ECE) Block - Rooms 101-105</option>
                        <option value="aids">Artificial Intelligence (AI DS / AI ML) Block</option>
                        <option value="eee">Electrical & Electronics (EEE / ICE) Block</option>
                        <option value="mech">Mechanical & Aero Engineering Workshops</option>
                        <option value="admin">Central Administration HQ & Library Complex</option>
                        <option value="clubs">Student Club Workspaces Zone</option>
                      </select>
                    </div>

                    <div className="md:col-span-2 flex items-center justify-center bg-cyber-dark/45 border border-cyber-blue/5 rounded-lg px-3 py-2 text-[9px] text-gray-400 uppercase text-center font-bold">
                      {mapDestination ? (
                        <span className="text-cyber-neon font-mono animate-pulse flex items-center gap-1">
                          🟢 INTERSECT ROUTE MATCHED • SPEED: 1.2ms
                        </span>
                      ) : (
                        <span>Ready for route query selection</span>
                      )}
                    </div>
                  </div>
                  ...
                  {mapDestination && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="p-3.5 bg-cyber-dark/85 border border-cyber-blue/25 rounded-lg space-y-3 font-sans"
                    >
                      {/* Active Router Visualization */}
                      <div className="flex items-center gap-2 justify-center flex-wrap py-2 border-b border-cyber-blue/10">
                        <span className="px-2.5 py-1 bg-cyber-blue/15 text-cyber-blue border border-cyber-blue/25 rounded text-[10px] font-mono font-bold uppercase shrink-0">
                          📍 START: {getStudentProfile(user.rollNumber).deptCode} WING
                        </span>
                        <span className="text-gray-500 font-extrabold">➔</span>
                        <span className="px-2.5 py-1 bg-cyber-slate/40 text-gray-400 border border-cyber-blue/5 rounded text-[10px] font-mono font-bold uppercase shrink-0">
                          CENTRAL PLAZA SPIRE
                        </span>
                        <span className="text-gray-500 font-extrabold">➔</span>
                        <span className="px-2.5 py-1 bg-emerald-950/40 text-cyber-neon border border-cyber-neon/20 rounded text-[10px] font-mono font-bold uppercase shrink-0 animate-pulse">
                          🎯 GOAL: {mapDestination.toUpperCase()} BLOCK
                        </span>
                      </div>

                      {/* Descriptive Route Info */}
                      <div className="space-y-1.5">
                        <span className="text-[9px] font-mono text-gray-400 uppercase tracking-wider block">🗣️ STEP-BY-STEP CAD DIRECTION INSTRUCTIONS:</span>
                        <p className="text-xs text-gray-300 leading-normal bg-cyber-blue/5 p-3 rounded border border-cyber-blue/10 font-mono text-[11px]">
                          {(() => {
                            const origin = getStudentProfile(user.rollNumber).deptCode;
                            const dest = mapDestination;

                            if (dest === "cse") {
                              if (origin === "CSE") return "You are already stationed in the CSE block (North Wing). Proceed to 3rd Floor, Central Annex.";
                              if (origin === "ECE") return "Exit the West Wing corridor, turn left and walk east across the main brick Central Plaza. Enter the 4-story CSE Block and take the central elevator to the 3rd Floor.";
                              return "Proceed past the Administrative Spire, head east to the computer labs Block. Climb to Room 301-305.";
                            }
                            if (dest === "ece") {
                              if (origin === "ECE") return "You are already stationed in the ECE block (West Wing). Labs L1-L3 are on your immediate right.";
                              if (origin === "CSE") return "Take the elevator down to the lobby, exit past the security node, head west across Central Plaza and enter the West Wing corridor on the first floor.";
                              return "Follow the central avenue west, passing the administrative center. ECE wing is located in the West complex.";
                            }
                            if (dest === "aids") {
                              return "Head to the East/North wing corner building (located directly adjacent to the CSE block). AI DS labs are on the 2nd Floor, and AI ML specialized labs are on the 3rd floor.";
                            }
                            if (dest === "eee") {
                              return "Go south-west past the robotics arena, EEE power engineering lab complexes are stationed inside the East wing extreme lower ground area. Instrumentation control sensor labs are located at Annex floor 2.";
                            }
                            if (dest === "mech") {
                              return "Follow the southern perimeter towards the heavy workshops (Zones H1-H3). The heavy machinery bays and the high-speed aerodynamics flight tunnel labs are stationed in the south wing.";
                            }
                            if (dest === "admin") {
                              return "Proceed directly to the Central Plaza Spire. The main registrar clearance counters and the central digital library occupy the ground floor and mezzanine wings.";
                            }
                            if (dest === "clubs") {
                              return "Enter the Central Plaza Annex. The Cyber Security Club base is stationed in Room 201, while the Robotics battleground labs occupy Room 105.";
                            }
                            return "Choose a corner destination from the dropdown to initialize navigation coordinates.";
                          })()}
                        </p>
                      </div>
                    </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 bg-cyber-slate/20 border border-cyber-blue/15 rounded-xl p-5 flex flex-col gap-4 relative overflow-hidden">
                    <div className="absolute inset-0 cyber-grid opacity-10" />
                    
                    <div className="flex justify-between items-center relative z-10">
                      <span className="font-mono text-[10px] text-cyber-blue uppercase tracking-widest block">Interactive Campus Skeleton Blueprint</span>
                      <span className="text-[9px] font-mono text-cyber-neon flex items-center gap-1 animate-pulse">
                        <span className="w-2 h-2 rounded-full bg-cyber-neon" /> SYSTEM TRACER ONLINE
                      </span>
                    </div>

                    {/* Interactive Skeleton View */}
                    <div className="border border-cyber-blue/20 rounded-xl bg-cyber-dark/85 p-4 relative z-10 min-h-[360px] flex flex-col justify-between">
                      {/* Campus Architecture Grid */}
                      <div className="grid grid-cols-3 gap-3">
                        {/* Column 1: West Wing */}
                        <div className="space-y-3">
                          <span className="text-[8px] font-mono uppercase text-gray-500 block tracking-widest border-b border-cyber-blue/10 pb-1">West Wing (Electronics)</span>
                          
                          {/* ECE Block */}
                          <div className={`p-2.5 rounded-lg border text-left transition-all ${
                            getStudentProfile(user.rollNumber).deptCode === "ECE" 
                              ? "bg-cyber-blue/20 border-cyber-neon text-cyber-neon shadow-md shadow-cyber-neon/10" 
                              : "bg-cyber-slate/25 border-cyber-blue/10 text-gray-400 hover:border-cyber-blue/35"
                          }`}>
                            <div className="flex justify-between items-start">
                              <span className="font-bold text-[10px] block font-display">ECE BLOCK</span>
                              <span className="text-[6px] font-mono bg-emerald-950 px-1 rounded text-cyber-neon font-bold">L1 - L3</span>
                            </div>
                            <p className="text-[8px] mt-0.5 text-gray-400">Electronics & Comm. Labs</p>
                            <span className="text-[7px] font-mono text-gray-500 block mt-1 uppercase">Room 101-108 • Tesla Advisor</span>
                            {getStudentProfile(user.rollNumber).deptCode === "ECE" && (
                              <div className="mt-1 bg-cyber-neon/10 text-cyber-neon font-mono text-[8px] px-1 py-0.5 rounded uppercase font-bold animate-pulse text-center">
                                📍 YOU ARE ENROLLED HERE ({getStudentProfile(user.rollNumber).class})
                              </div>
                            )}
                          </div>
                          ...
                          {/* EEE Block */}
                          <div className={`p-2.5 rounded-lg border text-left transition-all ${
                            getStudentProfile(user.rollNumber).deptCode === "EEE" 
                              ? "bg-cyber-blue/20 border-cyber-neon text-cyber-neon shadow-md shadow-cyber-neon/10" 
                              : "bg-cyber-slate/25 border-cyber-blue/10 text-gray-400 hover:border-cyber-blue/35"
                          }`}>
                            <div className="flex justify-between items-start">
                              <span className="font-bold text-[10px] block font-display">EEE BLOCK</span>
                              <span className="text-[6px] font-mono bg-blue-950 px-1 rounded text-cyber-blue font-bold">L4 - L5</span>
                            </div>
                            <p className="text-[8px] mt-0.5 text-gray-400">Electrical Power & Drives</p>
                            <span className="text-[7px] font-mono text-gray-500 block mt-1 uppercase">Power Station Lab • Faraday advisor</span>
                            {getStudentProfile(user.rollNumber).deptCode === "EEE" && (
                              <div className="mt-1 bg-cyber-neon/10 text-cyber-neon font-mono text-[8px] px-1 py-0.5 rounded uppercase font-bold animate-pulse text-center">
                                📍 YOU ARE ENROLLED HERE ({getStudentProfile(user.rollNumber).class})
                              </div>
                          ...
                          {/* ICE Block */}
                          <div className={`p-2.5 rounded-lg border text-left transition-all ${
                            getStudentProfile(user.rollNumber).deptCode === "ICE" 
                              ? "bg-cyber-blue/20 border-cyber-neon text-cyber-neon shadow-md shadow-cyber-neon/10" 
                              : "bg-cyber-slate/25 border-cyber-blue/10 text-gray-400 hover:border-cyber-blue/35"
                          }`}>
                            <div className="flex justify-between items-start">
                              <span className="font-bold text-[10px] block font-display">ICE BLOCK</span>
                              <span className="text-[6px] font-mono bg-purple-950 px-1 rounded text-purple-400 font-bold">L6 - L7</span>
                            </div>
                            <p className="text-[8px] mt-0.5 text-gray-400">Instrumentation Control</p>
                            <span className="text-[7px] font-mono text-gray-500 block mt-1 uppercase">Transducers & Sensors Center</span>
                            {getStudentProfile(user.rollNumber).deptCode === "ICE" && (
                              <div className="mt-1 bg-cyber-neon/10 text-cyber-neon font-mono text-[8px] px-1 py-0.5 rounded uppercase font-bold animate-pulse text-center">
                                📍 YOU ARE ENROLLED HERE ({getStudentProfile(user.rollNumber).class})
                              </div>
                        ...
                        {/* Column 2: Central Plaza / Administration */}
                        <div className="space-y-3">
                          <span className="text-[8px] font-mono uppercase text-gray-500 block tracking-widest border-b border-cyber-blue/10 pb-1">Central Spire</span>

                          {/* Administration Block */}
                          <div className="p-3 rounded-lg border bg-gradient-to-br from-cyber-blue/10 to-purple-950/10 border-cyber-blue/25 text-left transition-all hover:border-cyber-blue/50">
                            <div className="flex justify-between items-start">
                              <span className="font-display font-extrabold text-[10px] text-white">ADMIN HQ</span>
                              <span className="text-[6px] font-mono bg-cyber-blue/20 px-1 py-0.5 rounded text-cyber-blue font-bold uppercase">HQ</span>
                            </div>
                            <p className="text-[8px] mt-1 text-gray-300">Principal Office & Records</p>
                            <p className="text-[7px] font-mono text-gray-400 mt-1 leading-tight">Central Plaza Ground Floor Annex • Accounts Clearance desk</p>
                          </div>

                          {/* Clubs Workspaces Zone */}
                          <div className="p-3 rounded-lg border bg-cyber-slate/40 border-cyber-neon/20 text-left">
                            <span className="font-mono text-[8px] text-cyber-neon font-bold block uppercase tracking-wider">♣ CLUB WORKSPACES</span>
                            <div className="mt-1.5 space-y-1.5 text-[8px] text-gray-300 font-mono">
                              <div className="flex justify-between border-b border-cyber-blue/5 pb-1">
                                <span>Cyber Security Base</span>
                                <span className="text-cyber-neon font-bold">Room 201</span>
                              </div>
                              <div className="flex justify-between border-b border-cyber-blue/5 pb-1">
                                <span>Robotics Arena</span>
                                <span className="text-cyber-blue">Room 105</span>
                              </div>
                              <div className="flex justify-between border-b border-cyber-blue/5 pb-1">
                                <span>Fine Arts Hub</span>
                                <span className="text-pink-400">Plaza Annex</span>
                              </div>
                              <div className="flex justify-between">
                                <span>NSS Service Unit</span>
                                <span className="text-amber-400">Admin Yard</span>
                              </div>
                            </div>
                          </div>

                          {/* Student Locator Coordinate */}
                          <div className="p-2 bg-gradient-to-r from-cyber-blue/5 to-emerald-950/20 border border-cyber-neon/15 rounded-lg font-mono text-[8px] text-gray-400 text-center">
                            <span className="text-cyber-neon font-bold block uppercase tracking-widest text-[7px] mb-1">Your Class Coordinates</span>
                            <span>ROLL: {user.rollNumber}</span>
                            <span className="block text-white font-bold text-[9px] mt-0.5">{getStudentProfile(user.rollNumber).deptCode} Wing ({getStudentProfile(user.rollNumber).class})</span>
                          </div>
                        </div>

                        {/* Column 3: East & North Wings */}
                        <div className="space-y-3">
                          <span className="text-[8px] font-mono uppercase text-gray-500 block tracking-widest border-b border-cyber-blue/10 pb-1">East & North Wing (CSE/AI)</span>
                          
                          {/* CSE Block */}
                          <div className={`p-2.5 rounded-lg border text-left transition-all ${
                            getStudentProfile(user.rollNumber).deptCode === "CSE" 
                              ? "bg-cyber-blue/20 border-cyber-neon text-cyber-neon shadow-md shadow-cyber-neon/10 animate-pulse" 
                              : "bg-cyber-slate/25 border-cyber-blue/10 text-gray-400 hover:border-cyber-blue/35"
                          }`}>
                            <div className="flex justify-between items-start">
                              <span className="font-bold text-[10px] block font-display">CSE BLOCK</span>
                              <span className="text-[6px] font-mono bg-blue-950 px-1 rounded text-cyber-blue font-bold">T1 - T3</span>
                            </div>
                            <p className="text-[8px] mt-0.5 text-gray-400">Computer Science Lab Core</p>
                            <span className="text-[7px] font-mono text-gray-500 block mt-1 uppercase">Room 301-305 • Dr. Turing advisor</span>
                            {getStudentProfile(user.rollNumber).deptCode === "CSE" && (
                              <div className="mt-1 bg-cyber-neon/10 text-cyber-neon font-mono text-[8px] px-1 py-0.5 rounded uppercase font-bold text-center">
                                📍 YOU ARE ENROLLED HERE ({getStudentProfile(user.rollNumber).class})
                              </div>
                            )}
                          </div>
                          ...
                          {/* AI DS Block */}
                          <div className={`p-2.5 rounded-lg border text-left transition-all ${
                            getStudentProfile(user.rollNumber).deptCode === "AI DS" 
                              ? "bg-cyber-blue/20 border-cyber-neon text-cyber-neon shadow-md shadow-cyber-neon/10 animate-pulse" 
                              : "bg-cyber-slate/25 border-cyber-blue/10 text-gray-400 hover:border-cyber-blue/35"
                          }`}>
                            <div className="flex justify-between items-start">
                              <span className="font-bold text-[10px] block font-display">AI DS BLOCK</span>
                              <span className="text-[6px] font-mono bg-purple-950 px-1 rounded text-purple-400 font-bold">AI1 - AI2</span>
                            </div>
                            <p className="text-[8px] mt-0.5 text-gray-400">Artificial Intelligence & Data</p>
                            <span className="text-[7px] font-mono text-gray-500 block mt-1 uppercase">Neuro Networks Wing</span>
                            {getStudentProfile(user.rollNumber).deptCode === "AI DS" && (
                              <div className="mt-1 bg-cyber-neon/10 text-cyber-neon font-mono text-[8px] px-1 py-0.5 rounded uppercase font-bold text-center">
                                📍 YOU ARE ENROLLED HERE ({getStudentProfile(user.rollNumber).class})
                              </div>
                          ...
                          {/* AI ML Block */}
                          <div className={`p-2.5 rounded-lg border text-left transition-all ${
                            getStudentProfile(user.rollNumber).deptCode === "AI ML" 
                              ? "bg-cyber-blue/20 border-cyber-neon text-cyber-neon shadow-md shadow-cyber-neon/10 animate-pulse" 
                              : "bg-cyber-slate/25 border-cyber-blue/10 text-gray-400 hover:border-cyber-blue/35"
                          }`}>
                            <div className="flex justify-between items-start">
                              <span className="font-bold text-[10px] block font-display">AI ML BLOCK</span>
                              <span className="text-[6px] font-mono bg-purple-950 px-1 rounded text-purple-400 font-bold">ML1</span>
                            </div>
                            <p className="text-[8px] mt-0.5 text-gray-400">Machine Learning Wing</p>
                            <span className="text-[7px] font-mono text-gray-500 block mt-1 uppercase">Machine Training labs</span>
                            {getStudentProfile(user.rollNumber).deptCode === "AI ML" && (
                              <div className="mt-1 bg-cyber-neon/10 text-cyber-neon font-mono text-[8px] px-1 py-0.5 rounded uppercase font-bold text-center">
                                📍 YOU ARE ENROLLED HERE ({getStudentProfile(user.rollNumber).class})
                              </div>
                          ...
                          {/* MECH / CIVIL / AERO Engineering Blocks */}
                          <div className={`p-2 rounded-lg border text-left transition-all ${
                            (getStudentProfile(user.rollNumber).deptCode === "MECH" || getStudentProfile(user.rollNumber).deptCode === "AERO")
                              ? "bg-cyber-blue/20 border-cyber-neon text-cyber-neon shadow-md shadow-cyber-neon/10" 
                              : "bg-cyber-slate/25 border-cyber-blue/10 text-gray-400 hover:border-cyber-blue/35"
                          }`}>
                            <div className="flex justify-between items-start">
                              <span className="font-bold text-[9px] block font-display uppercase">MECH / AERO / CIVIL</span>
                              <span className="text-[6px] font-mono bg-amber-950 px-1 rounded text-amber-500 font-bold">H1 - H3</span>
                            </div>
                            <p className="text-[8px] mt-0.5 text-gray-400">Heavy Engineering Workshops & Aerodynamics Lab</p>
                            {getStudentProfile(user.rollNumber).deptCode === "MECH" && (
                              <div className="mt-1 bg-cyber-neon/10 text-cyber-neon font-mono text-[8px] px-1 py-0.5 rounded uppercase font-bold text-center animate-pulse">
                                📍 YOU ARE ENROLLED IN MECH
                              </div>
                            )}
                            {getStudentProfile(user.rollNumber).deptCode === "AERO" && (
                              <div className="mt-1 bg-cyber-neon/10 text-cyber-neon font-mono text-[8px] px-1 py-0.5 rounded uppercase font-bold text-center animate-pulse">
                                📍 YOU ARE ENROLLED IN AERO
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {/* Map status overlay */}
                      <div className="border-t border-cyber-blue/10 pt-3 flex items-center justify-between text-[10px] font-mono text-gray-400">
                        <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-cyber-blue" /> Interlocked LAN coordinates</span>
                        <span className="text-cyber-neon">CAD MAP v2.94 • CONFIRMED</span>
                      </div>
                    </div>

                    <div className="p-4 bg-cyber-dark/80 border border-cyber-blue/15 rounded-xl flex items-start gap-3 relative z-10">
                      <MapPin className="w-5 h-5 text-cyber-neon shrink-0 mt-0.5 animate-bounce" />
                      <div>
                        <h4 className="text-xs font-mono font-bold text-white uppercase tracking-wider">Designated Class & Wing Coordinates</h4>
                        <p className="text-[11px] text-gray-400 mt-1">
                          Student Initiate <strong className="text-white">#{user.rollNumber}</strong> is officially assigned to <strong className="text-cyber-blue">{getStudentProfile(user.rollNumber).department}</strong>.
                          Your classrooms, computer lab desks, and designated faculty advisor are stationed inside the <strong className="text-cyber-neon">{getStudentProfile(user.rollNumber).deptCode} WING</strong>. Please use this blueprint for physical coordination.
                        </p>
                      </div>
                    </div>
                  </div>

                  {/* Locations Detail List Panel */}
                  <div className="bg-cyber-slate/30 border border-cyber-blue/15 rounded-xl p-5 space-y-4">
                    <span className="font-mono text-[10px] text-cyber-blue uppercase tracking-widest block">Campus Directory Coordinates</span>
                    <div className="space-y-3.5 max-h-[420px] overflow-y-auto pr-2 custom-scrollbar">
                      {[
                        { type: "Department", name: "Computer Science & Engineering Block", rooms: "Labs T1-T5, Rooms 301-305", head: "Dr. Alan Turing", office: "Room 401, CSE Block" },
                        { type: "Department", name: "Electronics & Communication Block", rooms: "ECE Labs L1-L3, Rooms 101-105", head: "Dr. Nikola Tesla", office: "Room 210, ECE Block" },
                        { type: "Department", name: "Artificial Intelligence & Data Science Block", rooms: "AI Labs AI1-AI2, Room 412", head: "Dr. Grace Hopper", office: "Room 412, AI Block" },
                        { type: "Department", name: "Artificial Intelligence & Machine Learning Block", rooms: "ML Labs ML1, Room 415", head: "Dr. John McCarthy", office: "Room 415, AI Block" },
                        { type: "Department", name: "Electrical & Electronics Block", rooms: "Power Labs L4-L5, Room 215", head: "Dr. Michael Faraday", office: "Room 215, EEE Block" },
                        { type: "Department", name: "Instrumentation Control Block", rooms: "Sensors Labs L6-L7, Room 309", head: "Dr. Marie Curie", office: "Room 309, ICE Block" },
                        { type: "Department", name: "Mechanical Engineering Block", rooms: "Workshops H1-H2, Fluid Lab 110", head: "Dr. James Watt", office: "Room 110, Mechanical Block" },
                        { type: "Department", name: "Civil & Aerospace Engineering Block", rooms: "Aerodynamics Lab H3, Design Lab 120", head: "Dr. Wernher von Braun", office: "Room 120, Aerospace Block" },
                        { type: "Administration", name: "Administrative Headquarters", rooms: "Dean Office, Accounts, Admission Clearance", head: "Honorable Registrar", office: "Central Administration Block" },
                        { type: "Club Workspace", name: "Cyber Security Club (Cyber Knights Base)", rooms: "Room 201, Cyber Security Block", head: "Sarah Connor (Student Lead)", office: "CS Office 201" },
                        { type: "Club Workspace", name: "Robotics Club (RoboWarriors Hub)", rooms: "Room 105, Tech Block A", head: "James Smith (Student Lead)", office: "Robo Office 105" },
                        { type: "Club Workspace", name: "Fine Arts & Cultural Club", rooms: "Plaza Annex Room 12", head: "Clara Schumann (Student Lead)", office: "Fine Arts Room 12" },
                        { type: "Club Workspace", name: "Green Campus & Social Service Club", rooms: "NSS Unit Admin Yard Office", head: "Nelson Mandela (Student Lead)", office: "NSS Yard Office" }
                      ].map((item, idx) => (
                        <div key={idx} className="p-3 bg-cyber-dark/50 border border-cyber-blue/5 rounded-lg space-y-1 hover:border-cyber-blue/20 transition-all">
                          <div className="flex justify-between items-center text-[8px] font-mono uppercase tracking-widest font-bold">
                            <span className="text-gray-500">{item.type}</span>
                            <span className="text-cyber-blue">{item.office}</span>
                          </div>
                          <h4 className="text-xs font-sans font-bold text-white">{item.name}</h4>
                          <div className="text-[10px] font-mono text-gray-400 space-y-0.5">
                            <div>Rooms: <span className="text-gray-300">{item.rooms}</span></div>
                            <div>In-charge: <span className="text-cyber-neon font-semibold">{item.head}</span></div>
                          </div>
                        </div>
                      ))}
                    </div>
